...
export function ImportWizard({ open, onClose, onDone }: Props) {
  ...
  async function handleCommit() {
    ...
    setBusy(true);
    try {
      const r = await commitImport(file);
      setResult(r);
      setStep(3);
      onDone();
    } catch (err) {
      ...
      footer={
        step === 1 ? (
          ...
          <>
            ...
            >
              Import {preview?.summary.ok ?? 0} học sinh
            </Button>
          </>
        ) : (
      ...
      {step === 1 && (
        <div className="py-4">
          ...
          <button
            ...
            disabled={busy}
            className="flex w-full flex-col items-center gap-3 rounded-2xl border-[1.5px] border-dashed border-border-strong bg-surface-alt p-10 text-center transition-colors hover:border-brand"
          >
      ...
      {step === 3 && result && (
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          ...
          <p className="font-semibold text-text">
            Đã tạo {result.summary.ok} học sinh
          </p>
          <p className="text-sm text-text-muted">
            Bỏ qua {result.summary.duplicate} trùng, {result.summary.error} lỗi.
          </p>
