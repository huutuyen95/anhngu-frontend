...
import { ListChecks, Headphones, PenLine, FileUp } from "lucide-react";
...
type Kind = "multiple_choice" | "listening" | "writing" | "import";

const CARDS: { kind: Kind; label: string; icon: typeof ListChecks; wrap: string; desc: string; meta: string }[] = [
  { kind: "multiple_choice", label: "Trắc nghiệm", icon: ListChecks, wrap: "bg-skill-reading-soft text-skill-reading", desc: "Câu A/B/C/D, điền từ, True/False/Not Given.", meta: "Chấm tự động · thang 10 chia đều" },
  { kind: "listening", label: "Đề nghe", icon: Headphones, wrap: "bg-skill-listening-soft text-info", desc: "Gắn audio theo section, giới hạn số lần nghe.", meta: "Chấm tự động · đếm lần nghe ở máy chủ" },
  { kind: "writing", label: "Writing", icon: PenLine, wrap: "bg-skill-writing-soft text-success-bold", desc: "Đề viết luận có giới hạn từ và tiêu chí.", meta: "Cô chấm tay theo tiêu chí" },
...
export function CreateTestModal({ open, onClose, categoryId, onImport }: { open: boolean; onClose: () => void; categoryId?: number | null; onImport?: () => void }) {
  ...
  const [picked, setPicked] = useState<Kind>("multiple_choice");
  ...
  async function proceed() {
    ...
    setBusy(true);
    try {
      const skill = picked === "listening" ? "listening" : picked === "writing" ? "writing" : "reading";
      const { test } = await createTest({ title: "Đề thi mới", skill, category_id: categoryId ?? null });
